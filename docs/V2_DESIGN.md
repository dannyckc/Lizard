...
**Phase 3 — Locomotion (the big port). ⏳ REOPENED (2026-08-12) — being redone from scratch.**

> The first attempt landed as `c4a3b7c` and was taken back out of `main` the same day, to be rewritten rather than revised. It is parked whole on the **`v2-locomotion`** branch, where it still runs on top of Phases 4 and the UI port — read it there for the tables, the calibration and the gate.
>
> What came out of `main`: `Locomotor`, `Tread`, `Cadence`, `Bound`, `Footing`, `Crossing`, their two probes (`GaitProbe`, `PushProbe`) and the Gait drawer. What deliberately stayed, because it is stance geometry rather than a mover and Phase 4 is pinned to it: **`Carriage`** (the table, the projections, `stance_clearance`, `PERSPECTIVE`) and **`Attitude`** (the two axes, `rest` vs `active`), plus the armature's head-pinned solve and lateral wave, `Corpus.soundness`, and Poise's balance constraints.
>
> The body left behind stands, poses, droops, ragdolls and skins, and nothing travels it: `Creature2` keeps the shape of the tick with the locomotion step cut out, and the fields a mover writes (`speed`, `speed_norm`, `ang_vel`, `head_pos`, `armature.fore_carry`/`hind_carry`, `limb.foot_driven`) sit at their rest values as the seam to write against. The armature standing at pelvis 36.7 / withers 33.8 px off the rest stance alone — the same heights the old gait measured off its planted feet — is why there is still an animal on screen.
>
> The description below is the original plan, and stands as the plan for the rewrite.
